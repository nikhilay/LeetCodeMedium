 ...
 * (3) 0 < k ≤ 100, 0 < n ≤ 106, 0 < primes[i] < 1000.Ò
 */
public class SuperUglyNumber {
    public static void main(String[] args) {
        int[] input = {2, 7, 13, 19};
        System.out.println(new SuperUglyNumber().nthSuperUglyNumber(12, input));
    }
    ...
    public int nthSuperUglyNumber(int n, int[] primes) {
        ...
        for (int i = 1; i < n; i++) {
            ...
            for (int j = 0; j < primes.length; j++) {
                min = Math.min(min, primes[j] * memo[index[j]]);
            }
            memo[i] = min;
            for (int j = 0; j < primes.length; j++) {
                if (min == primes[j] * memo[index[j]]) {
                    index[j]++;
                }
            }
        }
